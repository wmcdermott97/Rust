...
fn string_verify(num: String) -> bool {
  ...
  // need to deal with adjacent twos still....
}
...
fn string_simplify(num: String) -> String {
  ...
  println!("In string_simplify beginning: {}", num);
  // remove decimal
  ...
  let (new1, new2) = new_num.split_at_mut(dec_pos - 1);
  return string_simplify(new1.to_string() + "." + &new2);
}